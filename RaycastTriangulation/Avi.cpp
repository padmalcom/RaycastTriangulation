...
Avi::Avi(std::string const& file_name, int frame_period, const WAVEFORMATEX* wfx) {
	...
	::AVIFileInit();
	HRESULT hr = ::AVIFileOpen(&pfile, (LPCWSTR)(file_name.c_str()), OF_WRITE | OF_CREATE, 0);
...
HRESULT Avi::add_wav(const char* src, DWORD flags) {
	...
	{
		HANDLE hf = CreateFile((LPCWSTR)src, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, 0, 0);
